...
const tickToHeight = (tick: number) => {
  const inverted = 10 - tick;
  return inverted * ((trackLength - ballSize - 1) / 10) + ballSize / 2 - 1;
};
...
const TickLabel = ({ tick }: { tick: number }) => (
  <div
    style={{
      ...
      left: "0px",
      top: `calc(${tickToHeight(tick) + 2}px - 0.5rem)`,
          ...
          {Array.from(Array(11).keys())
            ...
            .map((_, i) => (
              <div
                key={i}
                style={{
                  height: `${boldTicks.includes(i) ? boldTickHeight : tickHeight}px`,
                  width: `${scaleWidth}px`,
                  position: "absolute",
                  backgroundColor: "var(--fg-color)",
                  top: `${tickToHeight(i)}px`,
                }}
              ></div>
            ))}
        </div>
